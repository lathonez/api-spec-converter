    ...
    (obj.parameters || []).forEach((param, i) => {
        ...
        if (param.type === 'array') {
          ...
          if (style === 'matrix') {
            ...
            // There seems to be a bug here, not sure what the above logic around param.in is supposed to be doing.
            // This is getting collectionFormat = 'multi' applied to it, which is wrong
            // (we don't use this format anywhere)
            // {
            //     "name": "id",
            //     "in": "query",
            //     "type": "array",
...
Converter.prototype.convertSchema = function(def, operationDirection) {
    ...
    if (def.nullable) {
        // this is a hack and not part of the swagger2 spec - but it works for us and we've been using it in v2
        // https://stackoverflow.com/questions/38139657/nullable-fields-in-swagger-on-node-js
